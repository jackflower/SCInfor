...
void sitoEratostenesa(bool tab[], unsigned int n)
{
	...
	{              
		...
		if (!tab[i])
			for (unsigned int j = i*i; j <= n; j += i) //to wykreślamy jej wielokrotnosci
...
void sortuj(string &napis)
{
	...
	for (unsigned int i = 0, zamiany = 1; i<napis.length() - 1 && zamiany != 0; i++)
	{
		zamiany = 0;
		for (unsigned int j = 0; j<napis.length() - 1; j++)
