...
// | MathFunctions.cpp - imathematical function implementation |
...
namespace maths
{
	float PointToSegment(	const sf::Vector2f& point,
	...
	float SegmentToSegment(	sf::Vector2f& segA,
							sf::Vector2f& segB,
							sf::Vector2f& segC,
							sf::Vector2f& segD )
	{
		float distA = PointToSegment( segA, segC, segD );
		float distB = PointToSegment( segB, segC, segD );
		float distC = PointToSegment( segC, segA, segB );
		float distD = PointToSegment( segD, segA, segB );
...

	{
		...
		sf::Vector2f bisector = Normalize( angleBisector );

		float halfAngle =	angularWidth * 0.5f * 3.14f / 180.0f; 
		float forwardDist = Dot( angleToCircle, bisector ); //odleg³oœæ wzd³u¿ dwusiecznej œrodka okrêgu i wierzcho³ka k¹ta
    
		if (forwardDist < -circleRadius)
        
			return false;
    
		float rightDist =	abs(Dot(angleToCircle, VectorRightOf(bisector)));
