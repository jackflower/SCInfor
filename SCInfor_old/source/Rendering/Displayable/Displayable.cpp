...
namespace rendering
{
	namespace displayable
	{
		...
		{
			m_smooth = smoothing;
			Texture* p_texture = gResourceManager.GetTexture(texture_name);
			if (!p_texture)
			{
		...
		//Metoda ustawia teksturę obiektu
		void Displayable::setTexture(Texture *p_texture, bool smoothing)
		{
		...
		void Displayable::draw(sf::RenderWindow* render)
		{
			...
			if (m_animation_state)
			{
				...
				//tworzymy wskaźnik na obiekt klasy Texture (tekstura)
				...
				Texture *p_texture = gResourceManager.GetTexture(frame.getTextureName());
